    ...
    def search_or_get_bug(self, query):
        ...
        if query.isdigit():  # Fetch bug details if input is a bug ID
            bug_data = self.bugzilla_request(
                {
                    "id": query,
                    "include_fields": "id,summary,status,creation_time,assigned_to,product,component,description",
                }
            )
            if bug_data:
                bug = bug_data[0]
                output_str += "Bug Details:\n"
                ...
                output_str += (
                    f"Bug Link: {self.bugzilla_url}/show_bug.cgi?id={bug['id']}\n\n"
                )
        ...
        bugs_found = {
            bug["id"]: bug for bug in self.bugzilla_request({"quicksearch": query})
        }
        ...
        if sorted_bugs:
            ...
            for bug in sorted_bugs:
                ...
                output_str += f"   Created On: {bug['creation_time']}\n"
                output_str += (
                    f"   Assigned To: {bug.get('assigned_to', 'Unassigned')}\n"
                )
                output_str += (
                    f"   Bug Link: {self.bugzilla_url}/show_bug.cgi?id={bug['id']}\n\n"
                )
